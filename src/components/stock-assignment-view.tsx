...
import { User, StockItem, StockTransfer } from '@/types';
...
import { LoaderCircle, PlusCircle, Trash2, PackagePlus, History } from 'lucide-react';
...
const StockAssignmentView: React.FC<StockAssignmentViewProps> = ({ storeKeeper, allStockItems, allUsers, allStockTransfers }) => {
  ...
  const myTransfers = useMemo(() => {
      return allStockTransfers
        .filter(t => t.initiatedById === storeKeeper.id)
        .sort((a,b) => new Date(b.initiatedAt).getTime() - new Date(a.initiatedAt).getTime());
  }, [allStockTransfers, storeKeeper.id]);
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="mt-6">
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><History /> Transfer History</CardTitle>
                <CardDescription>A log of all your initiated stock transfers.</CardDescription>
            </CardHeader>
            <CardContent>
                <div className="rounded-md border">
                    <Table>
                        ...
                        <TableBody>
                            {myTransfers.length > 0 ? myTransfers.map(transfer => (
                                <TableRow key={transfer.id}>
                                    <TableCell>{format(new Date(transfer.initiatedAt), 'PPp')}</TableCell>
                                    <TableCell>{transfer.toBranch}</TableCell>
                                    <TableCell>{transfer.items.reduce((sum, item) => sum + item.imeis.length, 0)}</TableCell>
                                    <TableCell>
                                        <Badge variant={transfer.status === 'completed' ? 'success' : 'default'} className={cn(transfer.status === 'pending' && "bg-amber-500")}>
                                            {transfer.status}
                                        </Badge>
                                    </TableCell>
                                </TableRow>
