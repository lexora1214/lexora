...
import { LoaderCircle, UserPlus, CheckCircle, ShieldCheck } from "lucide-react";
...
interface AddRecoveryAdminViewProps {
  hrUser: User;
}
...
export default function AddRecoveryAdminView({ hrUser }: AddRecoveryAdminViewProps) {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "OTP Sent",
        description: `An OTP has been sent to the new recovery admin's mobile number (${mobileNumber}).`,
      });
      setStep('otp');
  ...
  const handleVerifyAndCreateUser = async (e: React.FormEvent) => {
    ...
    const tempAppName = `recovery-admin-signup-${Date.now()}`;
    ...
    try {
      ...
      await createUserProfile(
        ...
        undefined,
        {
            referrerId: hrUser.id 
        }
      );

      toast({
        title: "Recovery Admin Registered",
            ...
            <form onSubmit={handleVerifyAndCreateUser}>
              <div className="grid gap-4">
                  ...
                  <Button type="submit" className="w-full" disabled={isLoading}>
                      ...
                      Verify & Create Recovery Admin Account
                  </Button>
    ...
    <Card className="w-full max-w-lg mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <ShieldCheck />
          Register New Recovery Admin
        </CardTitle>
        <CardDescription>Enter the new Recovery Admin's details. They will be able to log in immediately.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSendOtp}>
          <div className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="email" type="email" placeholder="recovery.admin@example.com" required value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="grid gap-2">
